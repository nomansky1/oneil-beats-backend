...
// GET /beats — content-negotiated. Browsers + Googlebot get the SEO landing
// HTML; everyone else (mobile app, admin dashboard, SPA fetch with Accept */*,
// explicit application/json) gets the original JSON API. Listing 'application
// /json' first in req.accepts ensures Accept: */* defaults to JSON, preserving
// every existing API consumer. Only an explicit text/html preference (browser
// navigation) routes to the static landing page generated by build-beat-pages.
app.get('/beats', async (req, res) => {
  const wantsHtml = req.accepts(['application/json', 'text/html']) === 'text/html';
  if (wantsHtml) {
    const fs = require('fs');
    const htmlPath = path.join(__dirname, 'public', 'beats.html');
    if (fs.existsSync(htmlPath)) {
      return res.sendFile(htmlPath);
    }
    // Fall through to JSON if the static landing hasn't been generated yet
    // (e.g. dev environment without the Vercel build step).
  }
  try {
